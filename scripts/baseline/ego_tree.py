...
class ego_tree_unit(object):
    ...
    def __init__(self, demand, node_num, degree, max_degree):
        ...
        self.ego_tree = np.zeros(
            (math.ceil(node_num / 2), max_degree + 1, node_num))
    ...
    def change_insert(self):
        ...
        for i in range(math.ceil(self.node_num / 2)):
            ...
            while (self.demand[index, p_i[j]] > 0):
                #record insert
                if self.help[index * self.node_num + p_i[j],
                             0] == 0 and (p_i[j] in self.high):
                    self.help[index * self.node_num + p_i[j], 0] = 1
                    self.help[index * self.node_num + p_i[j],
                              1] = self.low[e_i]
                    #this is an undirected graph
                    self.help[index + p_i[j] * self.node_num, 0] = 1
                    self.help[index + p_i[j] * self.node_num,
                              1] = self.low[e_i]
                    if i % 2 == 0:
                        e_i -= 1
                    else:
            ...
            if self.ego_tree[i, 0, 0] > 0:
                for e in range(self.node_num):
                    if self.help[index * self.node_num + e, 0] == 1:
                        ...
                        if self.demand[index,
                                       self.help[index * self.node_num + e,
                                                 1]] == 0:
                            self.ego_tree[i, insert[0, 0] + 1, insert[
                                0, 1]] = self.help[index * self.node_num + e,
                                                   1]
                        if self.demand[index,
                                       self.help[index * self.node_num + e,
                                                 1]] > self.demand[index, e]:
                            self.ego_tree[i, insert[0, 0] + 1,
                                          insert[0, 1]] = -np.inf
                        if self.demand[index,
                                       self.help[index * self.node_num + e,
                                                 1]] < self.demand[index, e]:
