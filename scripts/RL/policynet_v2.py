...
class GnnPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=[dict(vf=[64,64])],
        ...
        with tf.variable_scope("gnn", reuse=reuse):
            ...
            gnnnet = model(self.n_nodes,self.max_degree,self.dims) #batch_size equals n_minibatch
    ...
    def _obs_process(self, obs):
        ...
        """
        obs = tf.reshape(obs,[-1,self.n_nodes*2+1,self.n_nodes])
        self.demand, self.adj, available_degree = tf.split(obs,[self.n_nodes,self.n_nodes,1],axis=-2)
        self.available_degree = tf.squeeze(available_degree, [-2])
