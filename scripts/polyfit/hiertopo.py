...
class HierTopoPolyn(object):
    ...
    def apply_policy(self, demand, alpha):
        ...
        for _ in range(self.n_maxstep):
            ...
            degree_mask = np.repeat(np.expand_dims(degree_full,0), self.n_node, 0) + np.repeat(np.expand_dims(degree_full, -1), self.n_node, -1)
            ...
            if (adj[add_ind] != 1) and (degree[add_ind[0]] < self.n_degree) and (degree[add_ind[1]] < self.n_degree):
    ...
    def apply_policy_w_replace(self, demand, alpha):
        ...
        adj = self.permatch_model.matching(demand, np.ones((self.n_node,)) * (self.n_degree - 1))
        ...
        for s in range(self.n_maxstep_force):
            ...
            for i in range(self.n_iter):
                exp_x = self.expand_orders_mat(x)
                weighing_self = np.matmul(exp_x, alpha[2*i*self.k:(2*i+1)*self.k])
                weighing_neigh = np.matmul(exp_x, alpha[(2*i+1)*self.k:(2*i+2)*self.k])
                neighbor_aggr = np.matmul(weighing_neigh, adj)
                g = weighing_self + neighbor_aggr
                #x = g/np.max(g)*2 # N x N
                gpos = np.where(g >= 0, g, z)
            ...
            if (degree[add_ind[0]] < self.n_degree) and (degree[add_ind[1]] < self.n_degree):
                G.add_edge(add_ind[0], add_ind[1])
            adj = np.array(nx.adjacency_matrix(G).todense(), np.float32)
            degree = np.sum(adj, axis=-1)
        
        if s == self.n_maxstep_force - 1:
            print("Maximum adjustement step ({}) reach. Unwillingly terminated.".format(self.n_maxstep_force))
    ...
    def apply_policy_w_replace_nsquare_list(self, demand, alpha):
        ...
        while remaining_choices:
            ...
            if degree[v1] >= self.n_degree and degree[v2] >= self.n_degree:
                ...
                v1_edges = np.where(np.array(v1_neighbor) > v1, v1 * self.n_node + np.array(v1_neighbor), np.array(v1_neighbor) * self.n_node + v1).tolist()
                ...
                v2_edges = np.where(np.array(v2_neighbor) > v2, v2 * self.n_node + np.array(v2_neighbor), np.array(v2_neighbor) * self.n_node + v2).tolist()
                ...
                if max(dif_e) + sum(self.cal_diff_in_range(v,rm_inds)) > sum(self.cal_diff_in_range(v_rp,[curr_e])) + sum(self.cal_diff_in_range(v_rp, rm_inds)):
                    G.remove_edge(int(e1_rm/self.n_node),e1_rm%self.n_node)
                    G.remove_edge(int(e2_rm/self.n_node),e2_rm%self.n_node)
                    G.add_edge(v1,v2)
                    adj = adj_rp
                    degree = np.sum(adj, axis=-1)
                    v = v_rp
...
def test_robust(solution, test_size, dataset, n_node, n_degree, n_iter, n_maxstep, k, ad_scheme):
    metrics = []
    ...
    for i in range(test_size):
        if ad_scheme == "add":
            ...
            m = hiertopo.apply_policy_w_replace_nsquare_list(dataset[i], solution)
        metrics.append(m)
...
def test_mp(solution, test_size, dataset, n_node, n_degree, n_iter, n_maxstep, k):
...
def main():
    ...
    parser.add_argument("-n", "--n_node", type=int, help="Number of nodes in the network", default=30)
    parser.add_argument("-d", "--n_degree", type=int, help="Degree limit for each node", default=4)
    parser.add_argument("-i", "--n_iter", type=int, help="Number of iterations", default=14)
    parser.add_argument("-np", "--n_node_param", type=int, help="Number of nodes in the network when the GNN model is trained", default=10)
    parser.add_argument("-ip", "--n_iter_param", type=int, help="Number of iterations when the GNN model is trained", default=10)
    parser.add_argument("-s", "--max_step", type=int, help="Maximum number of steps before topology adjustement ends", default=20)
    parser.add_argument("-k", type=int, help="Order of the local policy polynomial", default=3)
    parser.add_argument("-ds", "--data_source", type=str, help="The source of dataset", default="scratch", choices=["random", "nsfnet", "geant2", "scratch"])
    parser.add_argument("-a", "--ad_scheme", type=str, help="The topology adjustment scheme", default="replace", choices=["replace", "add"])
    ...
    if data_source == "random":
        ...
        file_demand = '../../data/2000_{0}_{1}_logistic.pk3'.format(n_node, n_degree)
    else:
        print("data_source {} unrecognized.".format(data_source))
        exit(1)

    file_logging = '../../poly_log/log{0}_{1}_{2}_{3}_same.pkl'.format(n_nodes_param,n_degree,k,n_iters_param)
    if ad_scheme == "replace":
        file_logging = '../../poly_log/log{0}_{1}_{2}_{3}_same_repl.pkl'.format(n_nodes_param,n_degree,k,n_iters_param)
    ...
    pred, pred_std = test_mp(solution, n_testings, dataset, n_node,n_degree, n_iter, n_maxstep, k)
